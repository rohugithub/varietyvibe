      ...
      <div className="bg-white border-b">
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-3xl md:text-4xl font-bold text-[#b74d80] mb-4">Frequently Asked Questions</h1>
            ...
            <div className="relative max-w-md mx-auto">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-[#b74d80]" />
              <Input type="search" placeholder="Search for answers..." className="pl-10 pr-4 py-3 w-full focus:ring-[#b74d80]" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-8xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {["50+", "24/7", "95%"].map((stat, i) => (
              <Card key={i} className="text-center hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  <div className="text-3xl font-bold text-[#b74d80] mb-2">{stat}</div>
                  <p className="text-gray-600">
                    {["Common Questions Answered", "Customer Support Available", "Questions Resolved Instantly"][i]}
                  </p>
                </CardContent>
              </Card>
            ))}
          ...
          <div className="space-y-8">
            {faqCategories.map((category) => (
              <Card key={category.id} className="hover:border-[#b74d80] transition-colors">
                <CardHeader>
                  <CardTitle className="flex items-center text-xl text-[#b74d80]">
                    <span className="text-2xl mr-3">{category.icon}</span>
                    {category.title}
                    <Badge variant="secondary" className="ml-3 text-white bg-[#b74d80]">
                  ...
                  <Accordion type="single" collapsible className="w-full">
                    {category.questions.map((faq, index) => (
                      <AccordionItem key={index} value={`${category.id}-${index}`}>
                        <AccordionTrigger className="text-left hover:text-[#b74d80] transition-colors">
                          {faq.question}
                        </AccordionTrigger>
          ...
          {/* Still Need Help */}
          <Card className="mt-12 bg-gradient-to-r from-[#b74d8020] to-[#b74d8033]">
            <CardContent className="p-8 text-center">
              <h3 className="text-2xl font-bold text-[#b74d80] mb-4">Still need help?</h3>
              <p className="text-gray-600 mb-6 max-w-2xl mx-auto">
                Can't find the answer you're looking for? Our customer support team is here to help you 24/7.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Button className="bg-[#b74d80] hover:bg-[#a03e6d] text-white">
                  <MessageCircle className="h-4 w-4 mr-2" />
                  Live Chat
                </Button>
                <Button variant="outline" className="hover:border-[#b74d80] hover:text-[#b74d80]">
